                    ...
                    <tbody class="HelveticaRegular">
                        <tr v-for="Aspirasi in paginatedPosts" class="w-full-screen text-[14px] even:bg-gray-200">
                            ...
                            <td scope="col" class="flex flex-col gap-3 px-6 py-5 font-normal">
                                <router-link :to="`/edit/aspirasi/${Aspirasi.id_aspirasi}`" class="w-[80px] text-center">✏️Edit</router-link>
                                <button @click="deleteAspirasi(Aspirasi.id_aspirasi)" class="w-[80px] cursor-pointer text-center">⛔Delete</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <router-link v-if="paginatedPosts.length === 0" to="/add/aspirasi" class="flex justify-center items-center HelveticaBold">
